...
const LAYERS = [
  {
    ...
    spread: { x: '-40%', y: '2%', rotate: -11, zIndex: 12 },
    widthClass: 'w-[min(467px,85vw)] max-w-[467px]',
  },
  {
    stack: { x: '-2%', y: '1%', rotate: 4, zIndex: 2 },
    spread: { x: 0, y: '-6%', rotate: 0, zIndex: 20 },
    widthClass: 'w-[min(434px,78vw)] max-w-[434px]',
  },
  {
    stack: { x: '-6%', y: '-6%', rotate: -6, zIndex: 3 },
    spread: { x: '38%', y: '4%', rotate: 10, zIndex: 12 },
    widthClass: 'w-[min(411px,82vw)] max-w-[411px]',
  },
] as const;
...
export function ProblemPhotoStack({ items, className }: Props) {
  ...
  return (
    <div
      className={cn(
        'relative mx-auto w-full max-w-[715px] min-h-[min(880px,120dvh)] outline-none',
    ...
    >
      <div className="pointer-events-none absolute left-1/2 top-[42%] min-h-[min(580px,70vw)] w-[min(467px,85vw)] -translate-x-1/2 -translate-y-1/2">
        {items.map((item, i) => {
          const cfg = LAYERS[i];
          ...
          return (
            ...
            >
              <motion.img
                ...
                height={item.height}
                draggable={false}
                className={cn('max-w-none rounded-sm', cfg.widthClass)}
                initial={false}
                animate={{
                  x: pose.x,
                  y: pose.y,
